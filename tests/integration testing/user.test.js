...
before (async() => {
    ...
    await db.none(seedFirstUser, testFirstUserEmail.email)
});
describe('User Routes', () => {
    ...
    it('Should check that OTP has been resent to user', function(done) {
            setTimeout(done, 10000)
            chai.request(server)
            .post('/api/v1/auth/resend-email-otp')
            .send({ email: 'someemail@some.email.com'})
            .then((err, res) => {
                console.log('Reset OTP response', res.body)
